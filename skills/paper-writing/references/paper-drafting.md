...
**Pitfall — template domain mismatch:** The references directory may contain templates from unrelated domains (e.g., a geology template when writing a social sciences paper). If the only template file doesn't match the paper's domain, do NOT force the paper into that template's structure. Instead, use the assignment/venue's specified structure directly. The template is a formatting guide, not a structural straitjacket.
...
When a paper has strict per-section word targets (common in course assignments), follow this procedure:

1. **After first draft, measure per-section word count** — use `sed -n '1,/^## References/p' draft.md | wc -w` for total, then split by `## ` headings to get per-section counts.
2. **Identify over/under sections** — compare each section against its target range.
3. **Prioritise targeted expansion/trimming** — expand the most underweight sections first (typically Discussion and Conclusion in first drafts); trim the most overweight sections (typically Results and Literature Review).
4. **Avoid wholesale edits** — use `patch` to replace specific paragraphs rather than rewriting entire sections, which risks destabilising word counts elsewhere.
5. **Re-measure after each batch of changes** — don't batch too many patches without checking; word count changes can compound unpredictably.
6. **Accept ±5% tolerance** — 6,000–6,500 is genuinely 6,000–6,500; ±100 words at the boundary is acceptable for most academic submissions.
